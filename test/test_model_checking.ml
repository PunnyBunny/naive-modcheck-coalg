...
let check_prob ~model_str ~formula_str ~point ~expected =
  ...
  let formula_ast = Logics.Probabilistic.parse_formula formula_str in
...
let test_prob_case_1 _ =
  check_prob ~model_str:prob_model_1 ~formula_str:"nu x . (p & <1/2 a> x)"
    ~point:"x" ~expected:true;
  check_prob ~model_str:prob_model_1 ~formula_str:"nu x . (~p & [1/2 b] ~x)"
    ~point:"x" ~expected:false

let test_prob_case_2 _ =
  check_prob ~model_str:prob_model_2 ~formula_str:"nu x . (p & <1/2 a> x)"
    ~point:"x" ~expected:true;
  check_prob ~model_str:prob_model_2 ~formula_str:"nu x . (~p & [1/2 b] ~x)"
    ~point:"x" ~expected:false
...
let rel_model_1 = {|[x: ({p1}, [{}: {x, y}]),
     y: ({p2}, [{}: {x, y}])]|}

let rel_model_2 = {|[x: ({ }, [{}: {x}, bubi: {y}]),
     y: ({ }, [])]|}

let test_rel_case_1 _ =
  check_rel ~model_str:rel_model_1
    ~formula_str:"nu x2 . (mu x1 . ((p1 & <> x1) | (p2 & [] x2)))" ~point:"x"
    ~expected:true
