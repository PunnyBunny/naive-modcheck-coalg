...
let rel_model_str = {|[x: ({p1}, [{}: {x, y}]), y: ({p2}, [{}: {x, y}])]|}
let rel_formula_str = "nu x2 . (mu x1 . ((p1 & <> x1) | (p2 & [] x2)))"
...
let setup _ctxt =
  ...
  let formula_ast = Logics.Relational.parse_formula rel_formula_str in
  let formula = Logics.Relational.formula_of_ast formula_ast in
  let game_data =
    Checkers.Relational.model_check_full ~verbose:false ~model
      ~point:(State.of_string "x") ~formula
  in
...
let test_json_structure ctxt =
  ...
  let json =
    Game_export.export_json ~game_data ~model_src:rel_model_str
      ~formula_src:rel_formula_str ~logic:"relational" ~point:"x"
  in
  ...
  assert_equal ~printer:Fn.id "relational" (meta |> member "logic" |> to_string);
  assert_equal ~printer:Fn.id "x" (meta |> member "point" |> to_string);
  assert_equal ~printer:Bool.to_string true (meta |> member "result" |> to_bool);
  assert_equal ~printer:Fn.id rel_formula_str
    (meta |> member "formula" |> to_string);
  (* Nodes *)
  let nodes = parsed |> member "nodes" |> to_list in
  assert_equal ~printer:Int.to_string game_data.num_nodes (List.length nodes);
  assert_bool "should have at least one node" (List.length nodes > 0);
  ...
  List.iter nodes ~f:(fun n ->
      ...
      assert_bool "states should be non-empty" (List.length states > 0));
  (* Edges *)
  let edges = parsed |> member "edges" |> to_list in
  assert_bool "should have edges" (List.length edges > 0);
  (* At least one strategy edge *)
  let has_strategy =
    List.exists edges ~f:(fun e -> e |> member "isStrategy" |> to_bool)
  in
  assert_bool "should have at least one strategy edge" has_strategy;
  (* Exactly one starting node *)
  let start_nodes =
    List.filter nodes ~f:(fun n -> n |> member "isStart" |> to_bool)
  in
  assert_equal ~printer:Int.to_string 1 (List.length start_nodes)

let test_game_data_result ctxt =
  let { model; formula; game_data } = bracket setup teardown ctxt in
  (* model_check_full result should match model_check *)
  let result =
    Checkers.Relational.model_check ~verbose:false ~model
      ~point:(State.of_string "x") ~formula
  in
  assert_equal ~printer:Bool.to_string result game_data.result;
