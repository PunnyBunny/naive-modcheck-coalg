...
struct
  ...
  type inner_node =
    ...
    | A_inner of A.inner_node (* Inner nodes of A *)
    | B_inner of B.inner_node (* Inner nodes of B *)
  ...
  let one_step_game ~(transition : M.state Model_spec.t)
    ...
    let convert_a_node a_formula (a_node : A.one_step_node)
        =
      match a_node with
    ...
    in
    let convert_b_node b_formula (b_node : B.one_step_node)
        =
      match b_node with
    ...
    let add_a_game (a_formula : M.formula A_closed.t)
        (a_game : A.one_step_game_t) =
      Hashtbl.iteri a_game
        ~f:(fun ~key:node ~data:(player, priority, succs) ->
    ...
    let rec add_a_games
      ...
      | Modal body ->
          let a_game =
            A.one_step_game ~transition:a_transition
              ~modal_formula:body
          in
    ...
    let rec add_b_games
      ...
      | Modal body ->
          let b_game =
            B.one_step_game ~transition:b_transition
              ~modal_formula:body
          in
