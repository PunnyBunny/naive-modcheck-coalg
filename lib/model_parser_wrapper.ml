...
module I =
  Model_parser.MenhirInterpreter (* name is usually MenhirInterpreter *)

let parse_model input (type t) (start : Lexing.position -> t I.checkpoint) : t =
  ...
  let rec loop (checkpoint : t I.checkpoint) =
    ...
    | HandlingError env ->
        ...
        failwith (Printf.sprintf "Parse error at line %d, column %d: %s, state=%d" line col message state)
  in
  loop checkpoint

let parse_relational_model (input : string) =
  parse_model input Model_parser.Incremental.relational_model
...
let parse_probabilistic_model (input : string) =
  parse_model input Model_parser.Incremental.probabilistic_model
