...
type game_data = {
  ...
  node_owners : string array;  (** ["Eloise" | "Abelard"] per node *)
  node_priorities : int array;
  node_successors : int list array;
  winners : string array;  (** ["Eloise" | "Abelard"] per node *)
  strategy : int option array;  (** Strategy edge target, if any *)
  node_is_modal : bool array;
  node_states : string list array;  (** State(s) per node: singleton for formula nodes, multiple for modal *)
  starting_node : int;
  result : bool;
}
(** Flat representation of a solved parity game for visualisation/export. *)
